...
if [ -f "${SCRIPT_DIR}/homebrew/shipd.rb" ]; then
    ...
    sed -i.bak "s|url \".*\"|url \"${TARBALL_URL}\"|g" "${SCRIPT_DIR}/homebrew/shipd.rb"
    sed -i.bak "s|sha256 \".*\"|sha256 \"${SHA256}\"|g" "${SCRIPT_DIR}/homebrew/shipd.rb"
    rm -f "${SCRIPT_DIR}/homebrew/shipd.rb.bak"
    echo -e "${GREEN}✓ Formula updated${NC}"
    echo ""
    echo "Changes to shipd.rb:"
    git diff "${SCRIPT_DIR}/homebrew/shipd.rb" || true
else
    echo -e "${YELLOW}⚠️  Formula not found at ${SCRIPT_DIR}/homebrew/shipd.rb${NC}"
...
echo "     git add homebrew/shipd.rb"
...
echo "     cp homebrew/shipd.rb /path/to/homebrew-tap/Formula/"
